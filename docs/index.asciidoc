...
| <<plugins-{type}s-{plugin}-array_remove_duplicate>> |<<array,array>>|No
...
[id="plugins-{type}s-{plugin}-array_remove_duplicate"]
===== `array_remove_duplicate`

  * Value type is <<array,array>>
  * There is no default value for this setting.

Removes duplicates from a field, so that only unique values remain.
