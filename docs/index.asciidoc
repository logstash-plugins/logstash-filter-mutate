...
Set the default value of a field that exists but is null. The default value can include `%{foo}` strings
...
then no action will be taken. The new value can include `%{foo}` strings
to help you build a new value from other parts of the event.
